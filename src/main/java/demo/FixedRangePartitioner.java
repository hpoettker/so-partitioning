...
public class FixedRangePartitioner implements Partitioner {
  ...
  public Map<String, ExecutionContext> partition(int gridSize) {
    ...
    try (InputStream is = new BufferedInputStream(new FileInputStream("src/main/resources/data.txt"))) {
    ...
    for (int i = 1; i <= gridSize; i++) {
      ...
      System.out.println("Index Final : " + (fromId + range - 1) + "\n");

      value.putInt("linesToSkip", fromId - 1);
      value.putInt("maxItemCount", range);
  ...
  public static void main(String[] args) {
    FixedRangePartitioner rangePartitioner = new FixedRangePartitioner();
